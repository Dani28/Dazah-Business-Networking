		...
		<div class="list-group">
			<?php foreach ($conversations AS $row): ?>
					<a id="conversation-<?= $row->conversation->id ?>" data-id="<?= $row->conversation->id ?>" data-user-id="<?= extract_user($row->conversation)->id ?>" class="col-lg-3 col-md-4 col-sm-6 list-group-item<?= (isset($row->archived_status) AND $row->archived_status) ? ' archived' : '' ?>" href="<?= conversation_url($row->conversation->id) ?>" data-toggle="tooltip" data-placement="right auto" title="Active <?= timestamp(extract_user($row->conversation)->usage->last_activity_timestamp) ?>">
						...
						<img src="<?= isset(extract_user($row->conversation)->thumbnail) ? extract_user($row->conversation)->thumbnail : '' ?>" alt="Profile Picture" class="img-circle img-thumbnail" width="32" height="32">
						<span class="h4">
