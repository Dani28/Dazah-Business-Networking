...
<?php if (!empty($conversations)): ?>
	<div id="active-conversations">
    	<?php foreach ($conversations AS $row): ?>
    		...
    		<a id="conversation-<?= $row->conversation->id ?>" data-id="<?= $row->conversation->id ?>" data-user-id="<?= extract_user($row->conversation)->id ?>" class="list-group-item<?= (isset($conversation) AND $conversation->id == $row->conversation->id) ? ' active' : '' ?>" href="<?= conversation_url($row->conversation->id) ?>" data-toggle="tooltip" data-placement="right" title="Last Seen <?= timestamp(extract_user($row->conversation)->usage->last_activity_timestamp) ?>">
    			<span class="archive"><span class="fa fa-times" aria-hidden="true"></span></span>
    			<span class="badge"><?= $row->new_message_count > 0 ? $row->new_message_count : '' ?></span>
    			...
    			<span class="h4">
    				...
    				<span><?= isset(extract_user($row->conversation)->profile->last_name) ? extract_user($row->conversation)->profile->last_name : 'Name #' . number_format($row->conversation->id) ?></span>
    			</span>
    		</a>
